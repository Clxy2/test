...
/**
 * Allow a fine tuned configuration of an EJB 3.0 EntityManagerFactory A Ejb3Configuration object is
 * only guaranteed to create one EntityManagerFactory. Multiple usage of
 * #buildEntityManagerFactory() is not guaranteed. After #buildEntityManagerFactory() has been
 * called, you no longer can change the configuration state (no class adding, no property change
 * etc) When serialized / deserialized or retrieved from the JNDI, you no longer can change the
 * configuration state (no class adding, no property change etc) Putting the configuration in the
 * JNDI is an expensive operation that requires a partial serialization
...
public class Ejb3Configuration implements Serializable, Referenceable {
    ...
    // made transient and not restored in deserialization on purpose, should no longer be called
    // after restoration
    ...
    /**
     * Used to inject a datasource object as the connection provider. If used, be sure to <b>not
     * override</b> the hibernate.connection.provider_class property
     */
    public void setDataSource(DataSource ds) {
        if (ds != null) {
            Map cpInjection = new HashMap();
            cpInjection.put("dataSource", ds);
            ((InjectionSettingsFactory) settingsFactory)
                    .setConnectionProviderInjectionData(cpInjection);
    ...
    /**
     * create a factory from a parsed persistence.xml Especially the scanning of classes and
     * additional jars is done already at this point.
     */
    private Ejb3Configuration configure(PersistenceMetadata metadata, Map overrides) {
        ...
        if (overrides != null) {
            for (Map.Entry entry : (Set<Map.Entry>) overrides.entrySet()) {
                ...
                props.put(entry.getKey(), value == null ? "" : value); // alter null, not allowed in
                // properties
            }
        }
    ...
    /**
     * Build the configuration from an entity manager name and given the appropriate extra
     * properties. Those properties override the one get through the peristence.xml file. If the
     * persistence unit name is not found or does not match the Persistence Provider, null is
     * returned This method is used in a non managed environment
    ...
    public Ejb3Configuration configure(String persistenceUnitName, Map integration) {
        try {
            ...
            integration = integration == null ? CollectionHelper.EMPTY_MAP : Collections
                    .unmodifiableMap(integration);
            ...
            while (xmls.hasMoreElements()) {
                ...
                List<PersistenceMetadata> metadataFiles = PersistenceXmlLoader.deploy(url,
                        integration, cfg.getEntityResolver(),
                        PersistenceUnitTransactionType.RESOURCE_LOCAL);
                for (PersistenceMetadata metadata : metadataFiles) {
                    log.trace("{}", metadata);

                    if (metadata.getProvider() == null
                            || IMPLEMENTATION_NAME.equalsIgnoreCase(metadata.getProvider())) {
                        // correct provider

                        // lazy compute the visitor if possible to avoid useless exceptions if an
                        // unexpected state happens
    ...
    private JarVisitor getMainJarVisitor(URL url, PersistenceMetadata metadata, Map integration) {
        ...
        Filter[] persistenceXmlFilter = getFilters(metadata, integration, metadata
                .getExcludeUnlistedClasses());
        return JarVisitorFactory.getVisitor(jarURL, persistenceXmlFilter);
    }

    private static void addMetadataFromVisitor(JarVisitor visitor, PersistenceMetadata metadata)
            throws IOException {
    ...
    private static void addScannedEntries(JarVisitor visitor, List<String> classes,
            List<String> packages, List<NamedInputStream> hbmFiles, List<String> mappingFiles)
            throws IOException {
    ...
    public Ejb3Configuration configure(PersistenceUnitInfo info, Map integration) {
        ...
        if (info.getClassLoader() == null) {
            throw new IllegalStateException(
                    "[PersistenceUnit: " + info.getPersistenceUnitName() == null ? "" : info
                            .getPersistenceUnitName()
        ...
        try {
            Map workingVars = new HashMap();
            workingVars.put(HibernatePersistence.PERSISTENCE_UNIT_NAME, info
                    .getPersistenceUnitName());
            ...
            boolean[] detectArtifactForOtherJars = getDetectedArtifacts(info.getProperties(), null,
                    false);
            ...
            Properties properties = info.getProperties() != null ? info.getProperties()
                    : new Properties();
            ...
            if ("true".equalsIgnoreCase(properties
                    .getProperty(HibernatePersistence.USE_CLASS_ENHANCER))) {
            ...
            if (integration.containsKey(HibernatePersistence.NON_JTA_DATASOURCE)) {
                String dataSource = (String) integration
                        .get(HibernatePersistence.NON_JTA_DATASOURCE);
                overridenDatasource = true;
                properties.setProperty(Environment.DATASOURCE, dataSource);
                if (isJTA == null)
                    isJTA = Boolean.FALSE;
            ...
            if (!overridenDatasource
                    && (info.getJtaDataSource() != null || info.getNonJtaDataSource() != null)) {
            ...
            /*
             * If explicit type => use it If a JTA DS is used => JTA transaction, if a non JTA DS is
             * used => RESOURCe_LOCAL if none, set to JavaEE default => JTA transaction
             */
    ...
    private void addXMLEntities(List<String> xmlFiles, PersistenceUnitInfo info,
        ...
        if (newTempClassLoader == null) {
            log
                    .warn("Persistence provider caller does not implement the EJB3 spec correctly. PersistenceUnitInfo.getNewTempClassLoader() is null.");
            return;
        }
        XMLHelper xmlHelper = new XMLHelper();
        List errors = new ArrayList();
        SAXReader saxReader = xmlHelper.createSAXReader("XML InputStream", errors, cfg
                .getEntityResolver());
        try {
        ...
        for (String xmlFile : xmlFiles) {
            ...
            try {
                ...
                if (errors.size() != 0) {
                    throw new MappingException("invalid mapping: " + xmlFile, (Throwable) errors
                            .get(0));
                }
                ...
                if (rootElement != null && "entity-mappings".equals(rootElement.getName())) {
                    ...
                    for (Element subelement : elements) {
                        String classname = XMLContext.buildSafeClassName(subelement
                                .attributeValue("class"), defaultPackage);
    ...
    private boolean[] getDetectedArtifacts(Properties properties, Map overridenProperties,
        ...
        String detect = overridenProperties != null ? (String) overridenProperties
                .get(HibernatePersistence.AUTODETECTION) : null;
        detect = detect == null ? properties.getProperty(HibernatePersistence.AUTODETECTION)
                : detect;
    ...
    private Filter[] getFilters(final boolean[] detectedArtifacts, final boolean searchORM,
        ...
        int size = (detectedArtifacts[0] ? 2 : 0)
                + ((searchORM || detectedArtifacts[1] || mappingFilesSize > 0) ? 1 : 0);
        ...
        if (detectedArtifacts[0]) {
            ...
            filters[1] = new ClassFilter(false, new Class[] { Entity.class, MappedSuperclass.class,
                    Embeddable.class }) {
                public boolean accept(String javaElementName) {
                    return true;
                }
    ...
    private void scanForClasses(URL jar, List<String> packages, List<String> entities,
            List<NamedInputStream> hbmFiles, boolean[] detectedArtifacts, boolean searchORM) {
        if (jar == null) {
    ...
    /**
     * create a factory from a list of properties and HibernatePersistence.CLASS_NAMES ->
     * Collection<String> (use to list the classes from config files
     * HibernatePersistence.PACKAGE_NAMES -> Collection<String> (use to list the mappings from
     * config files HibernatePersistence.HBXML_FILES -> Collection<InputStream> (input streams of
     * hbm files) HibernatePersistence.LOADED_CLASSES -> Collection<Class> (list of loaded classes)
     * <p/>
    ...
    /**
     * Process configuration and build an EntityManagerFactory <b>when</b> the configuration is
     * ready
     * @deprecated
     */
    ...
    public EntityManagerFactory buildEntityManagerFactory() {
        ...
        try {
            ...
            return new EntityManagerFactoryImpl(cfg.buildSessionFactory(), transactionType,
                    discardOnClose, getSessionInterceptorClass(cfg.getProperties()));
    ...
    private Class getSessionInterceptorClass(Properties properties) {
        String sessionInterceptorClassname = (String) properties
                .get(HibernatePersistence.SESSION_INTERCEPTOR);
    ...
    public Reference getReference() throws NamingException {
        ...
        return new Reference(Ejb3Configuration.class.getName(), new BinaryRefAddr("object",
                serialized), Ejb3ConfigurationObjectFactory.class.getName(), null);
    }
    ...
    private Ejb3Configuration configure(Properties properties, Map workingVars) {
            ...
            if (uncastObject != null && uncastObject instanceof String) {
                ...
                if (propertyKey.startsWith(HibernatePersistence.CLASS_CACHE_PREFIX)) {
                    ...
                    setCacheStrategy(propertyKey, preparedProperties, false, workingVars);
                        ...
                        && !(propertyKey.equals(HibernatePersistence.JACC_CONTEXT_ID) || propertyKey
                                .equals(HibernatePersistence.JACC_ENABLED))) {
                    jaccKeys.add(propertyKey);
                }
            }
        }
        if (preparedProperties.containsKey(HibernatePersistence.INTERCEPTOR)
                && (cfg.getInterceptor() == null || cfg.getInterceptor().equals(defaultInterceptor))) {
            // cfg.setInterceptor has precedence over configuration file
            String interceptorName = preparedProperties
                    .getProperty(HibernatePersistence.INTERCEPTOR);
            try {
                Class interceptor = classForName(interceptorName);
                cfg.setInterceptor((Interceptor) interceptor.newInstance());
            } catch (ClassNotFoundException e) {
                throw new PersistenceException(getExceptionHeader()
                        + "Unable to find interceptor class: " + interceptorName, e);
            } catch (IllegalAccessException e) {
                throw new PersistenceException(getExceptionHeader()
        ...
        if (preparedProperties.containsKey(HibernatePersistence.NAMING_STRATEGY)
                && (cfg.getNamingStrategy() == null || cfg.getNamingStrategy().equals(
                        defaultNamingStrategy))) {
            // cfg.setNamingStrategy has precedence over configuration file
            String namingStrategyName = preparedProperties
                    .getProperty(HibernatePersistence.NAMING_STRATEGY);
            try {
                ...
                throw new PersistenceException(getExceptionHeader()
                        + "Naming strategyy class does not implement NmaingStrategy interface: "
                        + namingStrategyName, e);
            }
        }
    ...
    private void addClassesToSessionFactory(Map workingVars) {
        if (workingVars.containsKey(HibernatePersistence.CLASS_NAMES)) {
            Collection<String> classNames = (Collection<String>) workingVars
                    .get(HibernatePersistence.CLASS_NAMES);
        ...
        if (workingVars.containsKey(HibernatePersistence.LOADED_CLASSES)) {
            Collection<Class> classes = (Collection<Class>) workingVars
                    .get(HibernatePersistence.LOADED_CLASSES);
        ...
        if (workingVars.containsKey(HibernatePersistence.PACKAGE_NAMES)) {
            Collection<String> packages = (Collection<String>) workingVars
                    .get(HibernatePersistence.PACKAGE_NAMES);
        ...
        if (workingVars.containsKey(HibernatePersistence.XML_FILE_NAMES)) {
            Collection<String> xmlFiles = (Collection<String>) workingVars
                    .get(HibernatePersistence.XML_FILE_NAMES);
            for (String xmlFile : xmlFiles) {
        ...
        if (workingVars.containsKey(HibernatePersistence.HBXML_FILES)) {
            Collection<NamedInputStream> hbmXmlFiles = (Collection<NamedInputStream>) workingVars
                    .get(HibernatePersistence.HBXML_FILES);
    ...
    private Properties prepareProperties(Properties properties, Map workingVars) {
        ...
        defineTransactionType(
                preparedProperties.getProperty(HibernatePersistence.TRANSACTION_TYPE), workingVars);
    ...
    private void setCacheStrategy(String propertyKey, Map properties, boolean isClass,
            Map workingVars) {
        String role = propertyKey.substring((isClass ? HibernatePersistence.CLASS_CACHE_PREFIX
                .length() : HibernatePersistence.COLLECTION_CACHE_PREFIX.length()) + 1);
        // dot size added
        String value = (String) properties.get(propertyKey);
        StringTokenizer params = new StringTokenizer(value, ";, ");
        if (!params.hasMoreTokens()) {
            StringBuilder error = new StringBuilder("Illegal usage of ");
            error.append(isClass ? HibernatePersistence.CLASS_CACHE_PREFIX
                    : HibernatePersistence.COLLECTION_CACHE_PREFIX);
    ...
    private void addSecurity(List<String> keys, Map properties, Map workingVars) {
        ...
        if (!properties.containsKey(HibernatePersistence.JACC_CONTEXT_ID)) {
            throw new PersistenceException(getExceptionHeader()
                    ...
                    + HibernatePersistence.JACC_CONTEXT_ID + " has not been set");
        }
        ...
        for (String key : keys) {
            ...
            try {
                ...
                throw new PersistenceException(getExceptionHeader() + "Illegal usage of "
                        + HibernatePersistence.JACC_PREFIX + ": " + key);
            }
        }
    }
    ...
    /**
     * This API is intended to give a read-only configuration. It is sueful when working with
     * SchemaExport or any Configuration based tool. DO NOT update configuration through it.
     */
    ...
    public Ejb3Configuration addResource(String path, ClassLoader classLoader)
